...
namespace Endorblast
{
    ...
    class PlayerMovement : Component, IUpdatable
    {
        float moveSpeed = 100;
        ...
        public void Update()
        {
            ...
            if (player.isMyPlayer)
            {
                ...
                if (keys != null)
                {
                    ...
                    if (keys._inputs[0] && keys._inputs[1])
                    {
                        state = PlayerState.Idle;
                        velocity.X = 0;
                    }
                    else if (keys._inputs[0] && !keys._inputs[1])
                    {
                        state = PlayerState.Running;
                        facingDir = false;
                        velocity.X = moveSpeed;
                    }
                    else if (keys._inputs[1] && !keys._inputs[0])
                    {
                        state = PlayerState.Running;
                        facingDir = true;
                        velocity.X = -moveSpeed;
                    ...
                    if (collisionState.Below && keys._inputs[2])
                    {
