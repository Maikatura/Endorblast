...
namespace Endorblast.Lib.TileMap
{
    public class TilemapHelper
    {
        ...
        private void LoadTileData()
        {
            ...
            string objectPath = contentPath + @"Objects\";
            List<string> objJsonPaths = Directory.GetFiles(objectPath, "*.json").ToList();
            for (int i = 0; i < objJsonPaths.Count; i++)
            {
                string jsonPath = objJsonPaths[i];
                TilemapObject objMap = TilemapObject.ObjectFromJson(jsonPath, graphicsDevice);
                objMap.ObjIndex = i;
                Console.WriteLine(objMap.Name);
                Objects.Add(objMap);
            }
